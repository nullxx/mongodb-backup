        ...
        collections.forEach(function(collection) {
            ...
            logger("select collection " + collection.collectionName), makeDir(name + collection.collectionName + path.sep, function(err, name) {
                ...
                meta(collection, metadata, function() {
                    collection.find(query).stream().once("end", function() {
...
function wrapper(my) {
    ...
    if ("function" == typeof my.parser) parser = my.parser; else switch (my.parser.toLowerCase()) {
      case "bson":
        BSON = require("bson"), parser = toBsonAsync;
        break;
    ...
    meta = !0 === my.metadata ? writeMetadata : function(a, b, c) {
        return c();
    }, require("mongodb-legacy").MongoClient.connect(my.uri, my.options, function(err, _db) {
        if (logger("db open"), err) return callback(err);
        var db = _db.db(), root = null === my.tar ? my.root : my.dir;
            ...
            makeDir(name + db.databaseName + path.sep, function(err, name) {
                function go() {
                    return discriminator(db, name, my.query, metadata, parser, function(err) {
                        if (logger("db close"), _db.close(), err) return callback(err);
                        my.tar ? makeDir(my.root, function(e, name) {
                            var dest;
                            err && error(err), dest = my.stream ? (logger("send tar file to stream"), my.stream) : (logger("make tar file at " + name + my.tar), 
                            fs.createWriteStream(name + my.tar));
                            var p = require("tar").c({
                                cwd: my.dir
                            }, fs.readdirSync(root));
                            p.pipe(dest), p.on("error", callback), p.on("end", function() {
